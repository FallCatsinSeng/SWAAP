...
func TestLoginExtractsPHPSESSIDFromWarmupCookieJar(t *testing.T) {
	...
	// Register all warmup chain handlers — the first one sets the PHPSESSID
	mux.HandleFunc("/swu.php", func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("swu"))
	})
	mux.HandleFunc("/my_school.php", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("school"))
	})
	mux.HandleFunc("/my_school_ok.php", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("ok"))
	})
	mux.HandleFunc("/my_school_run.php", func(w http.ResponseWriter, r *http.Request) {
...
func TestLoginFailsWhenSessionCookieUnavailable(t *testing.T) {
	// Server that never sets a PHPSESSID cookie — all warmup chain pages return OK but no cookie
	...
	if !strings.Contains(err.Error(), "no PHPSESSID obtained") {
